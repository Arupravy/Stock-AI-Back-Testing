...
if st.sidebar.button("Run Backtest"):
    with st.spinner("Running backtest..."):
        def backtest(stock_symbol, initial_capital, start_date, end_date, investment_type, strategy_type, transaction_cost=0.001, stop_loss_pct=0.05, take_profit_pct=0.10):
            ...
            # Output summary
            st.write(f"""
            ### Backtest Summary for {stock_choice.replace(" ", "_")}:            
            print(f"Strategy Type: {strategy_type}")
            print(f"Initial Capital: ${initial_capital:,.2f}")
            print(f"Final Portfolio Value: ${portfolio_value:.2f}")
            print(f"{'Profit' if total_return > 0 else 'Loss'}: ${abs(portfolio_value - initial_capital):,.2f}")
            ...
            print(f"Average Profit per Trade: {avg_profit_per_trade * 100:.2f}%")
            """)
            ...
            plt.plot(df['Portfolio_Value'], label='Portfolio Value (Strategy)', color='purple')
            plt.plot(df['HODL_Value'], label='HODL Strategy', color='orange', linestyle='--')
            ...
            plt.plot(df['Long_MA'], label=f'{long_window}-Day MA', color='red', linestyle='--')
            plt.plot(df['Upper_Band'], label='Upper Bollinger Band', color='orange', linestyle='--')
            plt.plot(df['Lower_Band'], label='Lower Bollinger Band', color='orange', linestyle='--')
            ...
            plt.title(f'{strategy_type} Strategy with Buy and Sell Signals')
            plt.xlabel('Date')
            plt.ylabel('Price')
            plt.legend()
            plt.show()
